...
public class GameController {
    ...
    public String buyGame(@PathVariable String id, Model model) {
        Game game = gameService.getGameById(id);
        if (game == null) {
            return "redirect:/games";
        }
        model.addAttribute("game", game);
        // Tampilkan halaman konfirmasi pembelian atau proses pembelian di sini
        return "buy-game";
    }
    
    @PostMapping("/games/buy/confirm/{id}")
    public String confirmBuy(@PathVariable String id, Model model) {
        Game game = gameService.getGameById(id);
        if (game == null) {
            return "redirect:/games";
        }
        // Di sini kamu bisa menambahkan logika update status game jika perlu
        model.addAttribute("game", game);
        return "buy-success";
    }
}